...
class Trainer:
    ...
    def __call__(self):
        ...
        divide_idx = int((hp.state_dim-5) / 2) #dimension of configuration space = 6D, #number of features = 5
            ...
            if episode_steps == self._episode_max_steps or done:
                ...
                for h in range(episode_steps):
                    ...
                    for k in range(hp.her_k):  # her_k = 4,  divide_idx = (19-5)/2 = 6D
                        ...
                        next_state[divide_idx:(hp.state_dim-5)] = future_next_state[:divide_idx]

                        if np.linalg.norm(next_state[:divide_idx] - next_state[divide_idx:(hp.state_dim-5)]) <= self._env.goal_bound:
                            reward = 0.0
                            done = True
                        else:
                            reward = -1.0
